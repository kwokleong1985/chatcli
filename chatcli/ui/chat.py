...
def chat_loop(sess: dict, mcp: Optional[McpManager] = None) -> None:
    ...
    console.print(
        ...
        f"thinking: {reasoning_label(sess.get('thinking', True), sess.get('reasoning_effort', 'default'))} · "
    ...
    while True:
        ...
        if cmd.startswith("/thinking"):
            ...
            console.print(
                f"[dim]Thinking: "
                f"{reasoning_label(sess['thinking'], sess.get('reasoning_effort', 'default'))}[/dim]"
            )
            continue
